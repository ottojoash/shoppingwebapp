...
// Define the type for product and cart items
interface Product {
  id: number;
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setProduct(data);
        setSelectedColor(data.colors[0]); // Default color
        setSelectedSize(data.sizes[0]); // Default size
      } catch (err: any) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white p-8 shadow rounded-lg">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-semibold mb-2">{product.name}</h1> {/* Changed from title to name */}
